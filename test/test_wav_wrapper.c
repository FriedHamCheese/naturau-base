...
static void test_WAVfile_to_ntrb_AudioHeader(FILE*, FILE*){
	...
	assert(WAVfile_to_ntrb_AudioHeader(&header, &audiodata_offset, &audiodata_size, file_buffer) == ntrb_AudioHeaderFromWAVFile_ok);
	...
	assert(WAVfile_to_ntrb_AudioHeader(&header, &audiodata_offset, &audiodata_size, file_buffer) == ntrb_AudioHeaderFromWAVFile_buffer_too_small);
	...
	assert(WAVfile_to_ntrb_AudioHeader(&header, &audiodata_offset, &audiodata_size, file_buffer) == ntrb_AudioHeaderFromWAVFile_invalid_RIFF_ID);
	free(file_buffer.ptr);
	...
	assert(WAVfile_to_ntrb_AudioHeader(&header, &audiodata_offset, &audiodata_size, file_buffer) == ntrb_AudioHeaderFromWAVFile_invalid_WAVE_ID);
	free(file_buffer.ptr);
	...
	assert(WAVfile_to_ntrb_AudioHeader(&header, &audiodata_offset, &audiodata_size, file_buffer) == ntrb_AudioHeaderFromWAVFile_invalid_fmt_ID);
	free(file_buffer.ptr);
	...
	assert(WAVfile_to_ntrb_AudioHeader(&header, &audiodata_offset, &audiodata_size, file_buffer) == ntrb_AudioHeaderFromWAVFile_invalid_data_ID);
	free(file_buffer.ptr);
	...
	assert(WAVfile_to_ntrb_AudioHeader(&header, &audiodata_offset, &audiodata_size, file_buffer) == ntrb_AudioHeaderFromWAVFile_invalid_chunk_size);
	free(file_buffer.ptr);	
	...
	assert(WAVfile_to_ntrb_AudioHeader(&header, &audiodata_offset, &audiodata_size, file_buffer) == ntrb_AudioHeaderFromWAVFile_invalid_Subchunk1Size);
	free(file_buffer.ptr);	
	...
	assert(WAVfile_to_ntrb_AudioHeader(&header, &audiodata_offset, &audiodata_size, file_buffer) == ntrb_AudioHeaderFromWAVFile_invalid_Subchunk2Size);
	free(file_buffer.ptr);
}
...
static void test_ntrb_get_WAV_audiodata(FILE*, FILE*){
	...
	assert(ntrb_read_entire_file_rb(&file_buffer, "test/wav_wrapper/regular_wav.wav") == ntrb_ReadFileResult_OK);
	assert(WAVfile_to_ntrb_AudioHeader(&header, &audiodata_offset, &audiodata_bytes, file_buffer) == ntrb_AudioHeaderFromWAVFile_ok);
...
void test_suite_ntrb_wav_wrapper(FILE* const outstream, FILE* const errstream){
	test_WAVfile_to_ntrb_AudioHeader(outstream, errstream);
